 ...
 */
public class Colours3DRender implements GLSurfaceView.Renderer {
    ...
    public Colours3DRender(Context context) {
        mContext = context;
    }
    ...
    private void init(Context context) {
        ...
        //编译两个着色器并链接在一起
        int vertexShader = ShaderHelper.compileVertexShader(TextReader.readTextFromResource(context, R.raw.vertex_shader_3d));
        int fragmentShader = ShaderHelper.compileFragmentShader(TextReader.readTextFromResource(context, R.raw.fragment_shader_3d));
...
//        } else {
//            Matrix.orthoM(mProjectionMatrix, 0, -1, 1, -1 / ratio, 1 / ratio, -1, 1);
//        }

        Matrix.perspectiveM(mProjectionMatrix, 0, 70, ratio, 1, 10);

        //由于顶点坐标没有z分量，这里要创建一个平移矩阵，平移，旋转
        Matrix.setIdentityM(mModelMatrix, 0);
        Matrix.translateM(mModelMatrix, 0, 0, 0, -2f);
        Matrix.rotateM(mModelMatrix,0,-60f,1,0,0);
